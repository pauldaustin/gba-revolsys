...
public class DistanceOp {
  ...
  private void computeContainmentDistance(final int polyGeomIndex,
    ...
    final List<Polygon> polys = GeometryUtil.getPolygons(this.geom[polyGeomIndex]);
    if (polys.size() > 0) {
      final List insideLocs = ConnectedElementLocationFilter.getLocations(this.geom[locationsIndex]);
  ...
  private void computeFacetDistance() {
     ...
     */
    final List<LineString> lines0 = GeometryUtil.getLines(this.geom[0]);
    final List<LineString> lines1 = GeometryUtil.getLines(this.geom[1]);

    final List<Point> pts0 = GeometryUtil.getPoints(this.geom[0]);
    final List<Point> pts1 = GeometryUtil.getPoints(this.geom[1]);
  ...
  private void computeMinDistance(final LineString line0, final LineString line1,
    final GeometryLocation[] locGeom) {
    if (line0.getBoundingBox().distance(line1.getBoundingBox()) > this.minDistance) {
      return;
    }
    ...
    for (int i = 0; i < coord0.size() - 1; i++) {
      for (int j = 0; j < coord1.size() - 1; j++) {
        final double dist = LineSegmentUtil.distance(coord0.get(i), coord0.get(i + 1),
          coord1.get(j), coord1.get(j + 1));
        if (dist < this.minDistance) {
          this.minDistance = dist;
          final LineSegment seg0 = new LineSegment(coord0.get(i), coord0.get(i + 1));
          final LineSegment seg1 = new LineSegment(coord1.get(j), coord1.get(j + 1));
  ...
  private void computeMinDistance(final LineString line, final Point pt,
    final GeometryLocation[] locGeom) {
    if (line.getBoundingBox().distance(pt.getBoundingBox()) > this.minDistance) {
      return;
    }
    ...
    // brute force approach!
    for (int i = 0; i < coord0.size() - 1; i++) {
      final double dist = LineSegmentUtil.distance(coord0.get(i), coord0.get(i + 1), coord);
      if (dist < this.minDistance) {
        this.minDistance = dist;
        final LineSegment seg = new LineSegment(coord0.get(i), coord0.get(i + 1));
  ...
  private void computeMinDistanceLinesPoints(final List lines, final List points,
    final GeometryLocation[] locGeom) {
