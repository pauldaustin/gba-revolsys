...
public class Triangle extends AbstractCoordinatesList {
  ...
  public Triangle(final Coordinates... points) {
    if (points.length > 3) {
      throw new IllegalArgumentException("A traingle must have exeactly 3 points not " + size());
    }
    for (int i = 0; i < 3; i++) {
  ...
  private void addIntersection(final GeometryFactory geometryFactory,
    final Set<Coordinates> coordinates, final Coordinates line1Start, final Coordinates line1End,
    final Coordinates line2Start, final Coordinates line2End) {
    final CoordinatesList intersections = LineSegmentUtil.getIntersection(geometryFactory,
  ...
  public Circle getCircumcircle() {
    ...
    final Coordinates centre = CoordinatesUtil.circumcentre(x1, y1, x2, y2, x3, y3);
