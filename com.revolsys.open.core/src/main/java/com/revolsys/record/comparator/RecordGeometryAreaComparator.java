...
public class RecordGeometryAreaComparator implements Comparator<Record> {
  ...
  public int compare(final Record object1, final Record object2) {
    ...
    if (compare == 0) {
      ...
      if (compare == 0) {
        final boolean clockwise1 = !CGAlgorithms
          .isCCW(geometry1.getExteriorRing().getCoordinates());
        final boolean clockwise2 = !CGAlgorithms
          .isCCW(geometry2.getExteriorRing().getCoordinates());
        if (clockwise1) {
          if (clockwise2) {
            return 0;
          } else {
