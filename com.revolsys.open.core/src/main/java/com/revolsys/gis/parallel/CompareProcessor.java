...
public class CompareProcessor extends AbstractMergeProcess {
  ...
  private void processExactLineMatch(final Record sourceObject) {
    ...
    final LineEqualIgnoreDirectionFilter lineEqualFilter = new LineEqualIgnoreDirectionFilter(
      sourceLine, 3);
    final Filter<Record> geometryFilter = new DataObjectGeometryFilter<LineString>(lineEqualFilter);
  ...
  @Override
  protected void processObjects(final RecordDefinition metaData, final Channel<Record> out) {
    ...
    this.otherIndex = new DataObjectQuadTree();
