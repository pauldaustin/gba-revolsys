...
public class QueryWhereConditionField extends ValueField
        ...
        if (fieldValue != null) {
          ...
          if (this.codeTable == null) {
            try {
              ...
              setInvalidMessage(fieldValue + " is not a valid " + attribute.getType());
              return;
            }
          } else {
            final List<Object> values = this.codeTable.getValues(fieldValue);
            if (values.size() == 1) {
  ...
  public void appendValue(final StringBuffer text, final Class<?> type, final Object value) {
