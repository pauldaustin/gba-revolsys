...
public class XbaseDataObjectWriter extends AbstractWriter<Record> {
  ...
  public XbaseDataObjectWriter(final RecordDefinition metaData, final Resource resource) {
  ...
  protected int addDbaseField(final String fullName, final DataType dataType,
    ...
    if (dataType == DataTypes.DECIMAL) {
      if (length > 18) {
        throw new IllegalArgumentException("Length  must be less < 18 for " + fullName);
      }
      field = addFieldDefinition(fullName, FieldDefinition.NUMBER_TYPE, length, scale);
    } else if (typeJavaClass == String.class) {
      if (length > 254 || length <= 0) {
        field = addFieldDefinition(fullName, FieldDefinition.CHARACTER_TYPE, 254);
      } else {
        field = addFieldDefinition(fullName, FieldDefinition.CHARACTER_TYPE, length);
      }
  ...
  protected FieldDefinition addFieldDefinition(final String fullName, final char type,
    final int length) {
  ...
  protected FieldDefinition addFieldDefinition(final String fullName, final char type,
    ...
    final FieldDefinition field = new FieldDefinition(name, fullName, type, length, decimalPlaces);
