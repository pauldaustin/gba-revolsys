...
public class ArcSdeBinaryGeometryRecordUtil {
  ...
  public ArcSdeBinaryGeometryRecordUtil() {
  }

  public ArcSdeBinaryGeometryRecordUtil(final RecordStore recordStore,
  ...
  public void createGeometryColumn(final AbstractJdbcRecordStore recordStore,
    ...
    GeometryFactory geometryFactory = JdbcFieldAdder.getColumnProperty(schema, typePath, columnName,
      JdbcFieldAdder.GEOMETRY_FACTORY);
  ...
  public AbstractIterator<Record> createIterator(final OracleRecordStore recordStore,
    ...
    if (boundingBox == null) {
      ...
      return new ArcSdeBinaryGeometryQueryIterator(this, recordStore, query, properties);
    }
  }
      ...
      try {
        ...
        if (row.getIndicator(columnIndex) != SeRow.SE_IS_NULL_VALUE) {
          ...
          switch (type) {
            ...
            default:
              LoggerFactory.getLogger(ArcSdeBinaryGeometryRecordUtil.class)
                .error("Unsupported column type: " + object.getRecordDefinition() + "." + name);
            break;
          }
